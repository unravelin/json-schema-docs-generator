...
Resolver.prototype.dereferenceSchema = function (schema, context, prop, stack) {
  ...
  if (stack && !_.isUndefined(prop)) {
    this._debug(3, 'Assigning "%s" to resolved schema: %s', prop, schema.docId || schema.title || 'No identifier');
    stack[prop] = schema;
  }
...
Resolver.prototype._normalizeReference = function (reference, context) {
  ...
  return resolvedSchema.docId + internalReference;
}
...
Resolver.prototype.resolve = function () {
  return _.map(this.schemas, _.bind(this.dereferenceSchema, this));
};
...
Resolver.prototype.addSchema = function (schema) {
  ...
  this.schemas[schema.docId] = schema;
  return this;
}
...
Resolver.prototype.removeSchema = function (schema) {
  delete this.schemas[schema.docId || schema];
  return this;
}
