...
Transformer.prototype.transform = function() {
  return _.mapValues(this.schemas, _.bind(this.transformSchema, this));
};
...
Transformer.prototype.transformLink = function(schema, link) {
  try {
    return _.extend(link, {
      htmlID: this._sanitizeHTMLID(link.title),
      ...
      request: link.schema ? this.formatData(this.generateExample(link.schema, schema)) : undefined
    });
  } catch (e) {
    throwError(e, 'Error building link for ' + chalk.yellow(schema.docId));
  }
};
...
Transformer.prototype.buildHref = function(href, schema, withExampleData) {
  ...
  try {
    ...
    throwError(e, 'Could not build href: ' + chalk.red(href));
  }
};
...
Transformer.prototype.buildCurl = function (link, schema) {
  var page = {};
  var headers = {};
  if (typeof this.schemaPages[schema.docId] !== 'undefined') {
    page = this.options.pages[this.schemaPages[schema.docId]];
  }
  var baseUrl = _.get(page, 'curl.baseUrl') || '';
  var uri = baseUrl + this.buildHref(link.href, schema, true);

  if (_.get(page, 'curl.requestHeaders')) {
    headers = _.get(page, 'curl.requestHeaders');
  }
